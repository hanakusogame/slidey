...
var MainScene = /** @class */ (function (_super) {
    ...
    function MainScene(param) {
        ...
        _this.loaded.add(function () {
            ...
            //スコア
            uiBase.append(new g.Sprite({ scene: _this, src: _this.assets["score"], x: 485, y: 15, height: 32 }));
            var score = 0;
            ...
            //同時消し表示
            var bkTweenCombo;
            _this.setCombo = function (num) {
                sprCombo.show();
                labelCombo.text = "" + num;
                labelCombo.invalidate();
                if (bkTweenCombo)
                    timeline2.remove(bkTweenCombo);
                bkTweenCombo = timeline2.create().every(function (e, p) {
                    sprCombo.opacity = p;
                    sprCombo.modified();
                }, 100).wait(4000).call(function () {
                    sprCombo.opacity = 0;
            ...
            //連鎖表示
            var bkTweenRen;
            _this.setRen = function (num) {
                sprRen.show();
                labelRen.text = "" + num;
                labelRen.invalidate();
                if (bkTweenRen)
                    timeline2.remove(bkTweenRen);
                bkTweenRen = timeline2.create().every(function (e, p) {
                    sprRen.opacity = p;
                    sprRen.modified();
                }, 100).wait(4000).call(function () {
                    sprRen.opacity = 0;
